...
const createTransaction = async (req, res) => {
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      console.debug('Create transaction validation failed. Payload:', req.body, 'Errors:', validationErrors);
      await t.rollback();
    ...
    if (transactionType === 'combined') {
      // Auto-generate transaction number for the anamath entry
      ...
      anamathEntry = await AnamathEntry.create({
        ...
        transactionNumber: nextTransactionNumber, // Add auto-generated transaction number
...
const approveTransaction = async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'manager' && isCredit) {
      await t.rollback();
      return res.status(403).json({ success: false, message: 'Managers can only approve debit transactions. Credit approval requires Admin.' });
    }
...
const updateTransaction = async (req, res) => {
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      console.debug('Update transaction validation failed. Payload:', req.body, 'Errors:', validationErrors);
      await t.rollback();
