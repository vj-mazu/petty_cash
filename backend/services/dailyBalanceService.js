...
class DailyBalanceService {
  ...
  async calculateClosingBalanceForDate(dateStr) {
    try {
      console.log('Calculating system-wide closing balance for date:', dateStr);
      ...
      if (openingBalanceRecords.length > 0) {
        // Use the sum of all closing balances from opening balance records
        const closingBalance = openingBalanceRecords.reduce((sum, balance) => {
          return sum + parseFloat(balance.closingAmount || 0);
        }, 0);
        
        console.log(`Closing balance for ${dateStr} from opening balance records: ${closingBalance}`);
      ...
      // Get all transactions for the specific date (system-wide)
      const transactions = await Transaction.findAll({
        where: {
          date: {
            [Op.between]: [
              startOfDay(parseISO(dateStr)),
              endOfDay(parseISO(dateStr))
            ]
          }
        },
      ...
      // Calculate total credits and debits for the day
      let totalCredits = 0;
      let totalDebits = 0;
      ...
      console.log(`Date: ${dateStr}, Opening: ${openingBalance}, Credits: ${totalCredits}, Debits: ${totalDebits}, Closing: ${closingBalance}`);
  ...
  async getOpeningBalanceForDate(dateStr) {
    try {
      console.log('Getting system-wide opening balance for date:', dateStr);
      ...
      if (!previousTransactions) {
        ...
        console.log('No previous transactions found, using system opening balance');
        return await this.getSystemOpeningBalance();
      }
      
      // Check if we have opening balance records for previous date
      const { OpeningBalance } = require('../models');
      ...
      if (previousDayBalances.length > 0) {
        // Sum all ledgers' closing balances from previous day
        const previousDayClosing = previousDayBalances.reduce((sum, balance) => {
          return sum + parseFloat(balance.closingAmount || 0);
        }, 0);
        
        console.log(`Opening balance for ${dateStr} based on previous day ${prevDateStr} closing: ${previousDayClosing}`);
        return previousDayClosing;
      }
      
      // Fallback: calculate from transaction history
      console.log('No opening balance records for previous day, calculating from transaction history');
      const prevClosing = await this.calculateClosingBalanceForDate(prevDateStr);
      console.log(`Opening balance for ${dateStr} = Previous day (${prevDateStr}) closing: ${prevClosing}`);
      return prevClosing;
      ...
      try {
        console.log('Error fallback: Using system opening balance');
        return await this.getSystemOpeningBalance();
      } catch (fallbackError) {
            ...
            if (prevBalance && currentBalance) {
              ...
              if (Math.abs(previousClosing - currentOpening) > 0.01) {
                ...
                const dayTransactions = await Transaction.findAll({
                  where: { ledgerId: ledger.id, date: currentDate }
                });
                let dailyCredits = 0;
                let dailyDebits = 0;
