      ...
      if (checkFileExists('migrations')) {
        ...
        if (migrationFiles.length > 0) {
          for (const migrationFile of migrationFiles) {
            ...
            // Note: In a real scenario, you'd want to track which migrations have been run
            // For now, we'll assume they're idempotent or can be safely re-run
          }
        ...
        console.log('ℹ️  Migrations directory not found');
      }
    } catch (error) {
      console.error('❌ Migration error:', error.message);
      // Don't exit here, migrations might have already been run
    }
