...
const CombinedTransactionForm: React.FC = () => {
  ...
  const onSubmit = async (formData: CombinedFormData) => {
    try {
      console.log('Combined transaction form submission started with data:', formData);
      ...
      console.log('Sending combined transaction data to API:', transactionData);
      console.log('DEBUG FRONTEND: transactionData being sent:', transactionData);
      const transactionResponse = await transactionApi.create(transactionData);
      console.log('Combined transaction API response:', transactionResponse);
      ...
      if (transactionResponse.success) {
        ...
        console.log('Combined transaction failure message:', transactionResponse.message);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className={`text-3xl font-bold ${colorScheme.text}`}>
      ...
      <motion.div
        ...
        className={`card p-8 bg-gradient-to-br ${colorScheme.bg} ${colorScheme.border}`}
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          {/* Combined Transaction Type Display */}
          <div className={`p-4 rounded-lg border-2 bg-${colorScheme.primary}-100 border-${colorScheme.primary}-300`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <IconComponent className={`w-6 h-6 mr-3 text-${colorScheme.primary}-600`} />
                <div>
                  <div className={`font-medium text-${colorScheme.primary}-900`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className={`p-6 rounded-xl border-2 bg-${colorScheme.primary}-50 border-${colorScheme.primary}-300`}>
              <h3 className={`text-lg font-semibold ${colorScheme.text} mb-4 flex items-center`}>
              ...
              <div className="space-y-4">
                {/* Ledger Selection */}
                <div>
                  <label htmlFor="ledgerId" className={`block text-sm font-medium text-${colorScheme.primary}-800 mb-2`}>
                    Select Ledger *
                  </label>
                  <div className="relative">
                    <select
                      ...
                      className={`input-field pl-10 pr-8 appearance-none bg-white border-${colorScheme.primary}-300 focus:border-${colorScheme.primary}-500 focus:ring-${colorScheme.primary}-500`}
                    >
                ...
                {/* Amount */}
                <div>
                  <label htmlFor="amount" className={`block text-sm font-medium text-${colorScheme.primary}-800 mb-2`}>
                ...
                {/* Date */}
                <div>
                  <label htmlFor="date" className={`block text-sm font-medium text-${colorScheme.primary}-800 mb-2`}>
                    Date *
                  </label>
                  <input
                    {...register('date', { required: 'Date is required' })}
                    type="date"
                    className={`input-field border-${colorScheme.primary}-300 focus:border-${colorScheme.primary}-500 focus:ring-${colorScheme.primary}-500`}
                  />
                  {errors.date && (
                ...
                {/* Description */}
                <div>
                  <label htmlFor="description" className={`block text-sm font-medium text-${colorScheme.primary}-800 mb-2`}>
            ...
            <div className="p-6 rounded-xl border-2 bg-amber-50 border-amber-300">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="anamathLedgerId" className="block text-sm font-medium text-amber-800 mb-2">
                ...
                {/* Anamath Amount */}
                <div>
                  <label htmlFor="anamathAmount" className="block text-sm font-medium text-amber-800 mb-2">
                    Anamath Amount *
                  </label>
                  <div className="relative">
                ...
                {/* Anamath Remarks */}
                <div>
                  <label htmlFor="anamathRemarks" className="block text-sm font-medium text-amber-800 mb-2">
                    Anamath Remarks *
                  </label>
                  <textarea
          ...
          {selectedLedger && watchedAmount > 0 && watchedAnamathAmount > 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="p-4 rounded-lg bg-amber-50 border border-amber-200">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center">
                      <Calculator className="w-5 h-5 mr-2 text-amber-600" />
                      <span className="font-medium text-amber-900">Anamath Entry</span>
                    </div>
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div className="text-amber-600">Balance Impact: None</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 p-3 bg-purple-100 rounded-lg border border-purple-200">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-purple-800">Total Amount Recorded:</span>
          ...
          <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
            ...
            <button
              ...
              className={`bg-${colorScheme.primary}-600 hover:bg-${colorScheme.primary}-700 text-white px-6 py-2 rounded-lg font-medium transition-colors duration-200 flex items-center disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              {isSubmitting ? (
                <>
