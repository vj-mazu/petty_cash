...
import { canEdit, canDelete, isAdmin, isManager, canApproveDebit, canApproveCredit } from '../utils/permissions';
...
const Transactions: React.FC = () => {
  ...
  const handleApprove = async (id: string) => {
    if (!isAdmin(user?.role) && !isManager(user?.role)) {
      toast.error('Only administrators and managers can approve transactions');
      return;
    }

    try {
      setIsApproving(id);
      // Wait for the api call (we need to add approve to transactionApi)
      const response = await (transactionApi as any).approve(id);
      if (response.success) {
  ...
  const [showAllTransactions, setShowAllTransactions] = useState<boolean>(false); // Default to show today only
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      ...
      const params = {
        page: 1,
        limit: hasFilters ? 50000 : 10000, // Higher limit when filtered for complete results
        ...
        no_count: 'true', // Always skip COUNT for ultra-fast performance
      ...
      if (txSuccess && openingBalanceSuccess) {
        ...
        if (ledgerSum.openingBalance && typeof ledgerSum.openingBalance === 'number') {
          ...
          console.log('Using system-wide opening balance from ledger summary:', systemOpeningBalance);
        } else {
          ...
          try {
            ...
            if ((perDate as any).success && (perDate as any).data && typeof (perDate as any).data.openingBalance === 'number') {
              ...
              console.log('Using opening balance from business date as fallback:', businessDateStr, systemOpeningBalance);
            }
          } catch (e) {
        ...
        // Fetch accurate opening balances for the ENTIRE date range for display continuity
        try {
          if (sortedTransactions.length > 0) {
            ...
            const balanceResponses = await Promise.all(allDatesInRange.map(async d => {
              try {
                const resp = await transactionApi.getBalances(d);
                return { d, ok: (resp as any).success !== false, val: (resp as any).data?.openingBalance };
              } catch (e) {
                console.warn('Failed to fetch balances for date', d, e);
                return { d, ok: false, val: undefined as number | undefined };
              }
            }));

            const map: Record<string, number> = {};
            for (const r of balanceResponses) {
              if (r.ok && typeof r.val === 'number') map[r.d] = r.val;
            }
            ...
            if (systemOpeningBalance === 0 && Object.keys(map).length > 0) {
              ...
              if (map[earliestDate] !== undefined) {
                ...
                console.log('Updated opening balance from earliest date:', earliestDate, map[earliestDate]);
              }
            }
          } else {
          ...
          console.warn('Failed to build openingByDate map:', e);
        }

        // Set total pages - handle different possible response structures
        let pages = 1;
        if (transactionsResponse.data && typeof transactionsResponse.data === 'object') {
          if ('pagination' in transactionsResponse.data && transactionsResponse.data.pagination && typeof transactionsResponse.data.pagination === 'object' && transactionsResponse.data.pagination !== null) {
            if ('pages' in transactionsResponse.data.pagination && typeof (transactionsResponse.data.pagination as any).pages === 'number') {
              pages = (transactionsResponse.data.pagination as any).pages;
            }
          } else if ('data' in transactionsResponse.data && typeof (transactionsResponse.data as any).data === 'object' && (transactionsResponse.data as any).data !== null) {
            const nestedData = (transactionsResponse.data as any).data;
            if ('pagination' in nestedData && nestedData.pagination && typeof nestedData.pagination === 'object' && nestedData.pagination !== null) {
              if ('pages' in nestedData.pagination && typeof (nestedData.pagination as any).pages === 'number') {
                pages = (nestedData.pagination as any).pages;
              }
            }
          }
        }
        ...
        // });
        console.log('Transactions: Totals state updated:', {
          totalDebit: ledgerSum.totalDebits,
          totalCredit: ledgerSum.totalCredits,
          balance: ledgerSum.totalBalance,
          openingBalance: systemOpeningBalance
        });
      }
    } catch (error: any) {
  ...
  // Handle refresh when coming from anamath creation
  useEffect(() => {
    ...
    try {
      ...
      if (currentDateIndex === -1 || currentDateIndex === 0) {
        ...
        for (let i = 0; i < currentDateIndex; i++) {
          ...
          const prevTransactions = transactionsByDate[prevDate] || [];
  ...
  const handleEditTransaction = (transaction: Transaction) => {
    ...
    console.log('Current user permissions:', { canEditTransactions }); // Add permission debugging
    ...
    console.log('Edit form data set:', {
      ...
      ledgerId: transaction.ledger?.id || '',
      date: toISODateString(transaction.date || new Date()),
      displayAmount: formatIndianNumber(amount, false)
    });
  };

  // Handle save edit
  const handleSaveEdit = async () => {
    console.log('handleSaveEdit called - DEBUG');
    ...
    console.log('Starting save edit process for transaction:', editingTransaction.id);
    ...
    try {
      ...
      console.log('Setting edit saving to false');
    }
  };
  ...
  const handleDeleteTransaction = async (transactionId: string) => {
    try {
      ...
      if ((response as any).success) {
        ...
        console.log('Delete failed with message:', delMsg);
  ...
  console.log('DEBUG: txNumberFilter from URL:', txNumberFilter);
  ...
  let filteredTransactions = txNumberFilter
    ? transactions.filter(t => {
      ...
      console.log(`DEBUG: Filtering transaction ID ${t.id}, TX#: ${num}, Search: ${txNumberFilter}`);
  ...
  if (allTransactionDates.length > 0) {
    ...
    for (const date of allTransactionDates) {
      ...
      const dayTransactions = transactions.filter(t => format(new Date(t.date), 'yyyy-MM-dd') === date);
    ...
    console.log('📊 Using backend opening balance for today:', todayStr, todayOpeningBalance);
  } else {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-6">
        {shouldShowEmptyTable ? (
          ...
          <div key={todayStr} className="bg-white rounded-lg shadow-lg overflow-hidden">
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                ...
                <p className="text-sm text-gray-600 mt-1">Opening: {formatCurrency(todayOpeningBalance)}</p>
              </div>
            </div>
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-1 px-1.5 bg-blue-50 font-medium text-gray-700 text-xs uppercase tracking-wider border-r border-gray-200" style={{ width: '60px' }}>
                      SL. NO
                    </th>
                    <th className="text-left py-1 px-1.5 bg-blue-50 font-medium text-gray-700 text-xs uppercase tracking-wider border-r border-gray-200" style={{ width: '100px' }}>
                      DATE
                    </th>
                    <th className="text-center py-1 px-1.5 bg-green-50 font-medium text-gray-700 text-xs uppercase tracking-wider border-r border-gray-200" style={{ width: '180px' }}>
                      CREDIT
                    </th>
                    <th className="text-center py-1 px-1.5 bg-red-50 font-medium text-gray-700 text-xs uppercase tracking-wider border-r border-gray-200" style={{ width: '180px' }}>
                      DEBIT
                    </th>
                    <th className="text-center py-1 px-1.5 bg-blue-50 font-medium text-gray-700 text-xs uppercase tracking-wider border-r border-gray-200" style={{ width: '100px' }}>
                      STATUS
                    </th>
                    <th className="text-center py-1 px-1.5 bg-blue-50 font-medium text-gray-700 text-xs uppercase tracking-wider" style={{ width: '100px' }}>
                      ACTIONS
                    </th>
                  </tr>
                  <tr className="border-b border-gray-300">
                    <th className="text-center py-1 px-1.5 bg-gray-100 font-medium text-gray-600 text-xs border-r border-gray-200">#</th>
                    <th className="text-center py-1 px-1.5 bg-gray-100 font-medium text-gray-600 text-xs border-r border-gray-200">DATE</th>
                    <th className="text-center py-1 px-1.5 bg-green-100 font-medium text-gray-600 text-xs border-r border-gray-200" colSpan={3}>
                    ...
                    <th className="text-center py-1 px-1.5 bg-red-100 font-medium text-gray-600 text-xs border-r border-gray-200" colSpan={3}>
                      <div className="grid grid-cols-3 gap-1">
                        <span>TX #</span>
                        <span>AMOUNT</span>
                        <span>LEDGER</span>
                    ...
                    <th className="text-center py-1 px-1.5 bg-gray-100 font-medium text-gray-600 text-xs border-r border-gray-200">STATUS</th>
                    <th className="text-center py-1 px-1.5 bg-gray-100 font-medium text-gray-600 text-xs">EDIT/DELETE</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td colSpan={9} className="px-6 py-8 text-center text-gray-500">
                      <div className="flex flex-col items-center space-y-4">
                        <div className="text-3xl text-blue-300 mb-2">🌅</div>
                        <p className="text-lg font-medium">Fresh Day - No Transactions Yet</p>
                        <p className="text-sm">Opening balance: {formatCurrency(todayOpeningBalance)}</p>
                        {(new Date().getHours() < 6 || (new Date().getHours() === 6 && new Date().getMinutes() < 30)) && (
          ...
          <>
            {(() => {
              ...
              return datesToShow.map(([date, dailyTransactions]) => {
                ...
                if (hasMany) {
                  console.warn(`⚠️ Date ${date} has ${sortedTransactions.length} transactions (recommended max: 120)`);
                }
                ...
                return (
                  ...
                  >
                    {/* Header with Date */}
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-blue-50 p-3 border-b border-gray-200 space-y-2 md:space-y-0">
                      <div>
                      ...
                      <div className="flex items-end space-x-2">
                        {date !== todayStr && (
                          <button
                            ...
                            className="px-2 py-1 text-xs rounded border border-blue-200 text-blue-700 hover:bg-blue-100"
                          >
                                ...
                                <td className="border border-gray-300 px-1.5 py-1 text-right">
                                  <div className="flex justify-end items-center space-x-1">
                                    {transaction.status === 'pending' && (() => {
                                      ...
                                      const canApprove = isCreditTx ? canApproveCredit(user?.role) : canApproveDebit(user?.role);
                                      return canApprove ? (
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                          ...
                                          title={isCreditTx ? 'Approve Credit (Admin)' : 'Approve Debit'}
                                        >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${editFormErrors.remarks ? 'border-red-500' : 'border-gray-300'
                    }`}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-8 pr-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${editFormErrors.amount ? 'border-red-500' : 'border-gray-300'
                      }`}
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${editFormErrors.ledgerId ? 'border-red-500' : 'border-gray-300'
                    }`}
                >
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${editFormErrors.date ? 'border-red-500' : 'border-gray-300'
                    }`}
                />
                {editFormErrors.date && (
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{transactionToDelete.ledger?.name}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Date:</span>
                    <span className="font-medium">{safeFormatDate(transactionToDelete.date, 'dd MMM yyyy')}</span>
                  </div>
                  ...
                  {transactionToDelete.reference && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{transactionToDelete.reference}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors disabled:bg-red-400 disabled:cursor-not-allowed flex items-center space-x-2"
                >
                  {isDeleting && (
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                  )}
