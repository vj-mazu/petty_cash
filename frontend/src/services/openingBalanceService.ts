...
export class OpeningBalanceService {
  ...
  public async calculateDailyOpeningBalance(ledgerId: string, date: string): Promise<OpeningBalance | null> {
    try {
      ...
      try {
        ...
        console.warn('Could not fetch previous day balance, using 0 as opening balance');
      }
  ...
  public async setManualOpeningBalance(
    ...
    try {
      ...
      if (response.success) {
        ...
        console.log(`Manual opening balance adjustment: ${amount} for ledger ${ledgerId} on ${date}. Reason: ${reason || 'No reason provided'}`);
  ...
  public async recalculateBalances(
    ...
    try {
      console.log(`Recalculating balances from ${startDate} to ${endDate}`);
