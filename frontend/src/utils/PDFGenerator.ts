...
class PDFGenerator {
  ...
  addTableWithBalanceTracking(
    ...
    try {
      console.log(`📊 PDFGenerator: Starting multi-page table with balance tracking`);
      console.log(`   Total transactions: ${transactions.length}`);
      console.log(`   Opening balance: ${options.openingBalance}`);
      ...
      pages.forEach((page, pageIndex) => {
        ...
        console.log(`   Page ${pageIndex + 1}: Opening ${pageOpeningBalance}, Credit ${pageCredit}, Debit ${pageDebit}, Closing ${pageClosingBalance}`);
      ...
      console.log(`✅ PDFGenerator: Multi-page table generated successfully`);
  ...
  addTable(columns: TableColumn[], rows: TableRow[]) {
    try {
            ...
            if (row[normalizedKey] !== undefined && row[normalizedKey] !== null) {
              ...
              if (matchingKey && row[matchingKey] !== undefined && row[matchingKey] !== null) {
                ...
                if (rowIndex < 3) { // Only log first few rows to avoid spam
                  console.log(`🔍 PDFGenerator: Missing cell data for column "${col.header}" (normalized: "${normalizedKey}") in row ${rowIndex}. Available keys:`, Object.keys(row));
                }
              }
            }
      ...
      console.log(`✅ PDFGenerator: Table generated successfully`);
    } catch (error) {
  ...
  generate(config: PDFConfig, filename: string) {
    try {
      console.log(`🔍 PDFGenerator: Starting PDF generation for "${filename}"`);
